...
pub fn next_prime(mut n: u64) -> u64 { if n <= 2 { return 2; } if n % 2 == 0 { n += 1; } while !is_prime(n) { n += 2; } n }
/// Previous prime strictly less than n
pub fn prev_prime(mut n: u64) -> Option<u64> { if n <= 2 { return None; } if n % 2 == 0 { n -= 1; } while n >= 2 && !is_prime(n) { n = n.saturating_sub(2); } if n >= 2 { Some(n) } else { None } }

/// n-th Fibonacci number (F(0)=0)
pub fn fibonacci(n: u32) -> u128 { let (mut a, mut b) = (0u128, 1u128); for _ in 0..n { let t = a + b; a = b; b = t; } a }

/// True if n is even
pub fn is_even(n: i64) -> bool { n % 2 == 0 }
/// True if n is odd
pub fn is_odd(n: i64) -> bool { n % 2 != 0 }

/// Convert degrees to radians
pub fn degrees_to_radians(deg: f64) -> f64 { deg.to_radians() }
/// Convert radians to degrees
pub fn radians_to_degrees(rad: f64) -> f64 { rad.to_degrees() }
...
pub fn euclidean_distance(a: &[f64], b: &[f64]) -> f64 { a.iter().zip(b).map(|(x,y)| (x-y).powi(2)).sum::<f64>().sqrt() }
/// Manhattan distance between equal-length vectors
pub fn manhattan_distance(a: &[f64], b: &[f64]) -> f64 { a.iter().zip(b).map(|(x,y)| (x-y).abs()).sum() }
/// Dot product of two vectors
pub fn dot_product(a: &[f64], b: &[f64]) -> f64 { a.iter().zip(b).map(|(x,y)| x*y).sum() }
/// 3D cross product
pub fn cross_product(a: [f64;3], b: [f64;3]) -> [f64;3] { [a[1]*b[2]-a[2]*b[1], a[2]*b[0]-a[0]*b[2], a[0]*b[1]-a[1]*b[0]] }
