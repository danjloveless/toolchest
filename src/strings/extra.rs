    ...
    while i < bytes.len() {
        if i + 3 < bytes.len() && &bytes[i..i + 2] == b"{{" {
            if let Some(end) = input[i + 2..].find("}}").map(|e| i + 2 + e) {
                ...
                if let Some(val) = provider(key) { out.push_str(&val); } else { out.push_str(&input[i..end + 2]); }
                i = end + 2;
                continue;
            }
        }
...
pub fn ensure_prefix(s: &str, prefix: &str) -> String {
    if s.starts_with(prefix) { s.to_string() } else { format!("{}{}", prefix, s) }
}

/// Ensure string has the suffix, adding if missing
pub fn ensure_suffix(s: &str, suffix: &str) -> String {
    if s.ends_with(suffix) { s.to_string() } else { format!("{}{}", s, suffix) }
}
...
pub fn is_palindrome(s: &str) -> bool {
    let filtered: String = s.chars().filter(|c| c.is_alphanumeric()).map(|c| c.to_ascii_lowercase()).collect();
...
pub fn indent(text: &str, prefix: &str) -> String {
    text.lines().map(|l| if l.is_empty() { "".to_string() } else { format!("{}{}", prefix, l) }).collect::<Vec<_>>().join("\n")
}
...
pub fn dedent(text: &str, n: usize) -> String {
    text.lines()
        .map(|l| {
            ...
            let mut idx = 0usize;
            for ch in l.chars() {
                if ch == ' ' && count < n { count += 1; idx += 1; } else { break; }
            }
            &l[idx..]
        })
        .collect::<Vec<_>>()
...
pub fn is_email(s: &str) -> bool { s.contains('@') && s.split('@').count() == 2 && s.split('@').nth(1).map_or(false, |d| d.contains('.')) }

/// Check if string looks like a URL (very basic)
pub fn is_url(s: &str) -> bool { s.starts_with("http://") || s.starts_with("https://") }
...
pub fn is_uuid(s: &str) -> bool {
    ...
    let lens = [8,4,4,4,12];
    if !parts.iter().zip(lens.iter()).all(|(p, &len)| p.len()==len && p.chars().all(|c| c.is_ascii_hexdigit())) { return false; }
    parts[2].starts_with('4')
}

/// Check if string is numeric (digits only)
pub fn is_numeric(s: &str) -> bool { !s.is_empty() && s.chars().all(|c| c.is_ascii_digit()) }

/// Repeat string n times
pub fn repeat(s: &str, n: usize) -> String { s.repeat(n) }

/// Replace all occurrences with callback providing replacement per match
pub fn replace_all<F>(s: &str, needle: &str, mut f: F) -> String where F: FnMut(&str) -> String {
...
pub fn split_words(s: &str) -> Vec<String> {
    ...
    for ch in s.chars() {
        if ch.is_whitespace() || ch == '_' || ch == '-' { if !current.is_empty() { words.push(current.clone()); current.clear(); } continue; }
        if ch.is_ascii_uppercase() && !current.is_empty() && current.chars().last().map_or(false, |c| c.is_ascii_lowercase()) {
...
pub fn longest_common_prefix(a: &str, b: &str) -> String {
    let mut out = String::new();
    for (ca, cb) in a.chars().zip(b.chars()) { if ca == cb { out.push(ca); } else { break; } }
    out
}
...
pub fn longest_common_suffix(a: &str, b: &str) -> String {
    let mut out = String::new();
    for (ca, cb) in a.chars().rev().zip(b.chars().rev()) { if ca == cb { out.insert(0, ca); } else { break; } }
    out
}

/// Shell-escape a string by wrapping in single quotes and escaping inner single quotes
pub fn escape_shell(s: &str) -> String { format!("'{}'", s.replace("'", "'\\''")) }

/// Random ASCII string from given charset
pub fn random_string(len: usize) -> String {
    let charset = b"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    let mut state = std::time::Instant::now().elapsed().as_nanos() as u128;
    let mut next_u64 = || {
        state = state.wrapping_mul(1664525).wrapping_add(1013904223);
        (state >> 32) as u64
    };
    let mut out = String::with_capacity(len);
    for _ in 0..len { let idx = (next_u64() as usize) % charset.len(); out.push(charset[idx] as char); }
    out
}

/// Mask part of a string, leaving prefix and suffix visible
pub fn mask(s: &str, prefix: usize, suffix: usize, mask_char: char) -> String {
    if s.len() <= prefix + suffix { return s.to_string(); }
    let mut out = String::new();
...
pub fn ellipsis_middle(s: &str, max_len: usize) -> String {
    if s.len() <= max_len || max_len < 3 { return s.to_string(); }
