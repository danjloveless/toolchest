...
where
    F: FnOnce(T) -> U,
{
    match res { Ok(v) => f(v), Err(_) => default }
}
...
pub fn try_cast<T: 'static, U: 'static + Clone>(_t: &T) -> Result<U, &'static str> { Err("unsupported without Any") }

/// Clamp x to [min, max]
pub fn coerce(x: i64, min: i64, max: i64) -> i64 { if x < min { min } else if x > max { max } else { x } }

/// Option helper: predicate on Some
pub fn is_some_and<T, F: FnOnce(&T) -> bool>(opt: Option<T>, f: F) -> bool { opt.as_ref().map_or(false, f) }
/// Result helper: predicate on Ok
pub fn is_ok_and<T, E, F: FnOnce(&T) -> bool>(res: Result<T,E>, f: F) -> bool { res.as_ref().map_or(false, |v| f(v)) }
/// Return reference or default
pub fn as_ref_or<'a, T>(opt: Option<&'a T>, default: &'a T) -> &'a T { opt.unwrap_or(default) }
/// Take Option value if predicate holds
pub fn take_if<T, F: FnOnce(&T) -> bool>(opt: &mut Option<T>, f: F) -> Option<T> { if opt.as_ref().map_or(false, |v| f(v)) { opt.take() } else { None } }
/// Replace value and return old
pub fn replace_with<T>(slot: &mut T, new_val: T) -> T { std::mem::replace(slot, new_val) }
/// Get the short type name without module path
pub fn type_name_short<T>() -> &'static str { std::any::type_name::<T>().rsplit("::").next().unwrap_or("") }
